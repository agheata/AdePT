...
const G4VPhysicalVolume *InitGeant4(const std::string &gdml_file)
{
  ...
  if (world == nullptr) {
    std::cerr << "example16: World volume not set properly check your setup selection criteria or GDML input!\n";
    return world;
  }
...
int main(int argc, char *argv[])
{
  ...
  copcore::DeviceSetStackLimit<copcore::BackendType::CUDA>(8192);
  ...
  tracer.setTag("callExample16");
  example16(particles, energy, batch, MCCindex, &scoringPerVolume, &globalScoring, NumVolumes, NumPlaced, &hepEmState,
            rotatingParticleGun);
